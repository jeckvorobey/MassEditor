...
- Релиз 1.1.0: добавлены остатки по складам, базовые характеристики, категории и выбор всех товаров по текущему фильтру
...
- изменение остатков по складам: установить, увеличить, уменьшить или сделать бесконечным
- базовое редактирование характеристик товара: установить существующее значение или очистить
- изменение категорий: добавить, удалить или заменить основную категорию
- применение операции ко всем товарам текущего фильтра с серверной повторной выборкой и лимитом операции
...
Для остатков доступны режимы установки, увеличения, уменьшения и отдельный режим бесконечного остатка.
Для характеристик в версии 1.1.0 доступны базовые общие характеристики товара: строка, текст, число и одиночное значение из списка. При установке значения плагин использует существующее значение характеристики или создаёт новое значение в таблице `shop_feature_values_*`. SKU-характеристики, множественные значения, цвета, диапазоны и размерности не входят в этот релиз.
Для категорий доступны добавление связи, удаление связи и замена основной категории с добавлением связи товара с категорией.

Поток операции:

1. пользователь отмечает товары или выбирает все товары по текущему фильтру
...
Если настройка ещё не сохранена, язык выбирается по текущей локали Webasyst через `wa()->getLocale()`: английские локали открывают английский интерфейс, остальные локали открывают русский интерфейс. После выбора языка в настройках сохранённое значение используется как явное переопределение.
...
Версия 1.1.0 не включает расширенные сценарии каталога:

- SKU-характеристики и характеристики модификаций;
- множественные значения характеристик, цвета, диапазоны, размерности;
- создание новых значений характеристик при массовой операции;
- массовую работу с фотографиями, видео, cross-selling, похожими товарами и страницами товаров;
- изменение storefront, корзины, заказов или темы магазина.

Остатки меняются через данные SKU и сохранение `shopProduct`; прямой массовой записи в `shop_product_stocks` в сервисе нет.
